...
vec3 drawCinematicNeon(vec2 uv, vec2 centerPos, vec3 color, float lineLength, float aspect, float textMask) {
    ...
    // Drastically narrowed glow to prevent wide overlapping haze (reduced another 20%)
    float ambientGlow = 0.00088 / (d * d + 0.04); 
    float intenseGlow = 0.00028 / (d * d + 0.002); 
...
void main() {
    ...
    // Deep red-orange base with reduced green channel to offset yellowing from additive blending
    vec3 finalColor = vec3(1.0, 0.15, 0.0) * 0.85 * smoothstep(-0.1, 1.0, vignette); 

    // --- 2. LIGHT SOURCES ---
    
    // STATIC BARS (Reduced length to ~0.2)
    finalColor += drawCinematicNeon(uv, vec2(0.15, 0.8), vec3(1.0, 1.0, 1.0), 0.2, aspect, mask);  // Pure White
    finalColor += drawCinematicNeon(uv, vec2(0.85, 0.15), vec3(0.8, 0.8, 0.8), 0.2, aspect, mask); // Light Grey
    finalColor += drawCinematicNeon(uv, vec2(0.1, 0.25), vec3(0.9, 0.95, 1.0), 0.15, aspect, mask); // Cool White
    finalColor += drawCinematicNeon(uv, vec2(0.9, 0.75), vec3(0.9, 0.9, 0.9), 0.15, aspect, mask); // Off-White

    // MOVING BARS (Wrapped from -2.0 to 2.0 for smooth off-screen transition)
    float t1 = fract(u_time * 0.03) * 4.0 - 2.0; 
    finalColor += drawCinematicNeon(uv, vec2(t1, 0.9), vec3(1.0, 0.95, 0.95), 0.22, aspect, mask); // Warm White

    float t2 = fract(u_time * -0.04 + 0.5) * 4.0 - 2.0; 
    finalColor += drawCinematicNeon(uv, vec2(t2, 0.1), vec3(0.85, 0.85, 0.9), 0.22, aspect, mask); // Cool Grey

    float t3 = fract(u_time * 0.025 + 0.2) * 4.0 - 2.0;
    finalColor += drawCinematicNeon(uv, vec2(t3, 0.55), vec3(0.95, 0.95, 0.95), 0.18, aspect, mask); // Bright Grey

    float t4 = fract(u_time * -0.035 + 0.8) * 4.0 - 2.0;
    finalColor += drawCinematicNeon(uv, vec2(t4, 0.4), vec3(0.75, 0.75, 0.75), 0.2, aspect, mask); // Grey
    ...
    if (mask > 0.01) {
        vec3 textGlowColor = vec3(1.0, 1.0, 1.0); // Pure White
        
        // Further tightened text radius to erase wide haze (reduced 25%+)
        vec3 neonGlow = textGlowColor * pow(mask, 5.0) * 0.35; 
        float coreMask = smoothstep(0.85, 1.0, mask);
        vec3 neonCore = vec3(1.0, 1.0, 1.0) * coreMask * 0.9; 
